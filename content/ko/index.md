...
Debian/Ubuntu에서는 `.deb`, Fedora에서는 `.rpm`을 사용하면 배포판이 사용자 네임스페이스를 제한하는 경우에도 Chromium 샌드박스를 사용할 수 있습니다. 설치 프로그램이 번들 샌드박스 도우미를 구성하므로 시스템 보안 설정을 변경할 필요가 없습니다.

AppImage는 다운로드한 파일을 실행 가능하게 만든 다음 여세요.
...
`error loading libfuse.so.2`가 표시되면 FUSE 없이 실행하세요.

```bash
./Paseo-x86_64.AppImage --appimage-extract-and-run
```

또는 Ubuntu 24.04 이상에서 `libfuse2t64`(`sudo apt install libfuse2t64`)를 설치하거나 배포판의 FUSE 2 호환 패키지를 설치하세요. 이 종속성은 Paseo가 시작되기 전 AppImage 런타임에 필요합니다.

Paseo는 시작할 때마다 샌드박스 가용성을 확인합니다. 사용자 네임스페이스가 작동하면 AppImage와 압축을 푼 tar 아카이브에서도 샌드박스를 유지합니다. 제한된 호스트에 사용 가능한 설치형 도우미가 없으면 Chromium 샌드박스를 비활성화한 상태로 시작합니다. OS 프로세스 격리가 필요하면 설치 패키지를 사용하세요. **설정 → 진단 → 앱 진단**은 샌드박스 상태와 이유를 보고하며 데스크톱 로그에도 같은 결정이 기록됩니다. 명시적인 `--no-sandbox` 인수는 자동 선택보다 우선합니다.
