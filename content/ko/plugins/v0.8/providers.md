...
호스트 범위 플러그인 설정을 지속하려면 `server.registerSettings()`가 반환한 핸들을 사용하세요. 카탈로그와 세션 콜백에서 설정을 읽고, 공급자가 구성을 메모리에 유지할 때는 변경 사항을 구독하세요. `read()`와 `subscribe()` 계약은 [설정](./reference.md#persisted-values)을 확인하세요.
