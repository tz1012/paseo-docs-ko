...
호스트 측 CLI 명령은 컨테이너를 명시적으로 선택합니다. 예: `paseo project ls --host 127.0.0.1:6767`. 엔드포인트 선택자가 없으면 CLI는 로컬 홈의 슈퍼바이저를 찾습니다. 컨테이너 환경 설정은 배포 재정의이며 워커를 재시작해도 유지됩니다. 전체 슈퍼바이저 교체는 컨테이너 관리자가 담당합니다.
