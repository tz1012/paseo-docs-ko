...
데몬은 변경을 적용하기 전에 전체 파일을 검증합니다. 런타임에 안전한 변경을 적용하고, 재시작이 필요한 설정을 나열합니다. 재시작이 필요한 경로를 보고하면 다음을 실행하세요.

```bash
paseo daemon restart
```

런타임에 안전한 설정에는 릴레이 활성화, MCP 설정, 브라우저 도구, 호스트 이름, CORS 출처, 신뢰할 수 있는 프록시, Git 프로세스 제한, 에이전트 및 터미널 프로필, 공급자 정의, 메타데이터 생성, 앱 기본 URL, 공급자 카탈로그 시간 초과, 전역 플러그인 스위치가 포함됩니다. 이러한 설정 중 하나를 제거하면 필드가 생략됐을 때의 동작이 적용되며, 공급자를 제거하면 이후 실행에서 해당 공급자가 제거됩니다.

새 Paseo 홈에서는 `daemon.relay.enabled`를 제거하면 릴레이가 비활성화됩니다. 시작할 때부터 이 필드가 없었던 데몬은 호환성을 위해 기존의 릴레이 활성화 동작을 유지합니다. 이전 구성을 편집할 때는 `daemon.relay.enabled`를 명시적으로 설정하세요.
...
그러면 비밀번호를 묻는 메시지가 표시되고 bcrypt 해시가 `config.json`에 기록되며 데몬을 다시 시작하라는 메시지가 표시됩니다. 인증은 시작 설정이므로 `paseo reload`도 재시작이 필요하다고 보고합니다.
