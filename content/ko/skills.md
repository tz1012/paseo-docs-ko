...
Paseo는 코딩 에이전트에게 Paseo 도구 및 CLI를 사용하여 다른 에이전트를 생성, 조정 및 관리하는 방법을 가르치는 오케스트레이션 기술을 제공합니다. Skills는 일반적인 워크플로를 슬래시 명령으로 패키지하므로 상담원은 매번 브리핑 및 안전 레일을 작성하지 않고도 조정 방법을 알 수 있습니다.
...
데스크톱 앱이 설치된 Paseo 기술을 찾으면 시작 시 번들 기술을 최신 상태로 유지합니다. 자동 업데이트가 실패하는 경우 설정 → 통합 → 업데이트 또는 위의 수동 명령을 사용하세요.

## `/paseo`, Paseo 참조

기초 스킬. 에이전트 및 작업 영역 관리를 위한 Paseo 참조입니다. 에이전트가 에이전트를 생성하거나 메시지를 보내거나 작업 공간 격리를 관리해야 할 때 로드하세요.
...
수신 상담원은 작업, 컨텍스트, 관련 파일, 현재 상태, 시도한 내용, 결정, 승인 기준 및 제약 조건이 포함된 자체 브리핑을 받습니다. 공급자는 이름을 지정하지 않는 한 오케스트레이션 기본 설정에서 제공됩니다. 요청 시 작업 트리 격리 작업 공간을 지원합니다.
