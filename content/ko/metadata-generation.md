...
**설정 → 호스트 → 메타데이터**를 열고 **자동** 또는 **수동**을 선택하세요. 수동 모드에서 Paseo는 선택한 모델을 먼저 시도한 다음, 나머지 구성된 모델과 내장 후보를 차례로 시도합니다. 모델을 선택하면 `agents.metadataGeneration.providers`의 첫 번째 항목을 교체하고 목록의 나머지는 유지합니다.

## 사용자 지정 대체 순서 구성

선호 모델을 둘 이상 구성하거나 정확한 순서를 제어하려면 `~/.paseo/config.json`에서 `agents.metadataGeneration.providers`를 설정하세요. 기본 제공 기본값보다 먼저 항목이 시도됩니다.
...
설정 화면은 첫 번째 항목만 교체하며 사용자 지정 목록의 나머지는 유지합니다. 파일을 직접 편집한 후 데몬을 다시 시작하세요.
