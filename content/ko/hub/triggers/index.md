...
에이전트가 작업 중이면 새 프롬프트가 현재 작업의 방향을 조정합니다. 각 도착 이벤트는 자체 출력 제한과 완료 상태를 유지합니다. 재사용 가능한 작업공간이 보관되어 있으면 Hub는 프롬프트를 보내기 전에 Paseo에 복원을 요청합니다.
...
키와 에이전트 재사용은 [연속 실행 참조](/docs/hub/configuration/hub-yml#agent-continuation)에서 다룹니다. 실행 세부정보에는 각 도착 이벤트가 에이전트를 생성했는지, 이어서 사용했는지, 복원했는지가 표시됩니다.
