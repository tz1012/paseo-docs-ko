...
데몬을 연결하는 것만으로 Hub에 에이전트 실행 권한이 부여되지는 않습니다. Hub 자동화에서 데몬을 사용하려면 `hub.execute`를 부여하세요. 이 권한을 통해 Hub는 해당 데몬에서 에이전트를 생성하고 이어서 실행하며, 에이전트의 상태와 타임라인을 살펴보고, 모델·사고·공급자 권한 모드를 구성하고, 작업공간을 보관하거나 복원할 수 있습니다. 공급자 모드에는 승인 프롬프트 우회가 포함될 수 있습니다. 기존 에이전트와 작업공간에도 적용되므로 Hub가 만든 작업만 격리하는 경계가 아닙니다.

데몬 관리, 터미널 제어, 데몬 자격 증명 권한 관리에는 별도의 권한이 필요합니다. Hub에 자체 권한을 변경할 수 있는 능력을 부여하지 않고도 `hub.execute`를 취소할 수 있습니다.
