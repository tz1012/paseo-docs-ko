...
후속 요청은 활성 에이전트의 실행 기한이나 자격 증명 만료 시간을 연장하지 않고 현재 작업의 방향을 조정합니다. `run.github` 권한 부여 또는 `run.env`의 연결 토큰이 있으면 후속 요청은 활성 에이전트의 자격 증명을 공유합니다. 모든 요청이 완료되면 Hub는 대여한 토큰을 폐기하며, 다음 도착 이벤트는 새 자격 증명으로 새 에이전트를 시작합니다. 임시 자격 증명이 없는 에이전트는 완료 후 재사용할 수 있습니다.
