...
데몬은 Paseo 데몬을 실행하는 머신 중 하나입니다. Hub 조직에 한 번 등록하면 트리거에서 이를 참조할 수 있습니다.
...
대화형 터미널에서 로그인하면 이 데몬을 연결할지 제안하고, Hub 자동화가 에이전트를 실행하도록 허용할지 별도로 묻습니다. 연결의 기본값은 예이고 실행 권한의 기본값은 아니요입니다. 그런 다음 코드 기반 시작용 트리거를 만들 수 있도록 **Triggers**로 연결하고 `paseo hub init`을 출력합니다. `--json` 또는 비 TTY 로그인은 로그인만 수행합니다. [빠른 시작](/docs/hub/quickstart)에서 연결과 초기화를 설명합니다.
